...
export default function AgentsGrid({ onSelectAgent, onChat }: AgentsGridProps) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#0d0d1a]">
            ...
            {rows.map((row, idx) => {
              ...
              return (
                <section key={idx}>
                  ...
                  >
                    <h2 className="text-xs font-medium text-gray-500 group-hover:text-gray-300 transition-colors">
                      {row.title}
                    </h2>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-gray-600">{row.agents.length} agents</span>
                      <svg
                  ...
                  {isExpanded && (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-px bg-[#1e1e32] border border-[#1e1e32] rounded-lg overflow-hidden">
                      {row.agents.map((agent) => (
                        <button
                          key={agent.id}
                          onClick={() => onSelectAgent(agent)}
                          className="text-left bg-[#12121f] p-5 hover:bg-[#1a1a2e] transition-colors group"
                        >
                          <div className="flex items-center gap-2.5 mb-2">
                            <div className="w-8 h-8 rounded-lg bg-purple-600/10 border border-purple-500/20 flex items-center justify-center text-purple-400 flex-shrink-0 group-hover:bg-purple-600/20 transition-colors">
                              {getAgentIcon(agent.id)}
                            </div>
                            <h3 className="text-sm font-semibold text-purple-400 group-hover:text-purple-300 transition-colors">
                              {agent.name}
                            </h3>
                          </div>
                          <p className="text-[11px] text-gray-500 leading-relaxed line-clamp-2 group-hover:text-gray-400 transition-colors pl-[42px]">
                            {agent.description}
                          </p>
                        </button>
                      ))}
      ...
      <div className="flex-shrink-0 border-t border-[#1e1e32] bg-[#12121f] px-10 py-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-3 border border-[#2a2a3e] rounded-xl px-4 py-3 bg-[#0d0d1a] focus-within:border-purple-500/40 transition-colors">
